<template>
  <div class="content">
    ...
    <el-dialog title="场地选择" :visible.sync="entry.fieldChoice" width="70%" height="70%" :close-on-click-modal="false" :append-to-body="true">
    ...
    <el-dialog title="品类选择" :visible.sync="entry.categoryChoice" width="70%" height="70%" :close-on-click-modal="false" :append-to-body="true">
    ...
    <el-dialog title="品牌选择" :visible.sync="entry.brandChoice" width="70%" height="70%" :close-on-click-modal="false" :append-to-body="true">
...
<script>
  ...
  export default {
    data() {
      return {
        approvalTable:[],//操作记录
        approvalTableHeight: window.innerHeight - 135, //表格高度
        create: {
          mall: {
            mall_id: '', //门店Id
    ...
    props: ['createDataTypeId', 'createDataType'],
    watch: {
      createDataTypeId: function(newVal, oldVal) {

      },
      createDataType(newVal, oldVal){
        console.log(newVal)
      }
    },
    created() {
    ...
    methods: {
      ...
      statusName() {
        const routerParams = this.createDataType;
        const routerType = this.createDataTypeId;
        ...
        if(this.createDataTypeId) {
          common.getARlist('ApprovalRecord/getARlist', {menu:'contract_main',id:this.createDataTypeId}, this)
        }
      },
      view(data, type) {
        let that = this;
      ...
      avoidType() {//免租方式
        let that = this;
        this.axios.post(process.env.BASE_API +'DecorateDays/getDays', {
      ...
      residentialPool(data){
        ...
        return  this.axios.post(process.env.BASE_API + 'property_unit/getCoefficient', { property_unit_id: data }).then((res) =>  res.data)
          .catch((err) => {
          that.$message.error(err.response.data.msg);
        });
      },
