  ...
  <div>
        ...
        <el-col :span="6">
          <el-form-item label="所属级别">
            <el-select v-model="userForm.mall_rank" placeholder="请选择">
              ...
              <el-option value="1" label="直营店"/>
              <el-option value="2" label="合资店"/>
              <el-option value="3" label="加盟店"/>
            </el-select>
          </el-form-item>
        </el-col>
        <el-col :span="12">
    ...
    <el-table
      ...
      <el-table-column
        ...
        label="所属级别">
        <template slot-scope="scope">
          <span v-if="scope.row.mall_rank == 1">直营店</span>
          <span v-else-if="scope.row.mall_rank == 2">合资店</span>
          <span v-else-if="scope.row.mall_rank == 3">加盟店</span>
        </template>
      </el-table-column>
      <el-table-column
        :show-overflow-tooltip="true"
...
export default {
  ...
  data() {
    return {
      userForm: { mall_name: '', mall_rank: '' }, // 表单
      tableData: [], // table
      userData: [], // 获取的所有的用户信息
  ...
  methods: {
    search() { // 搜索
      ...
      common.getPreData(data, 'mallInfo/getMallInfoList', this, 'mallList');
      common.set('mallList', { 'search': data });
    },
