...
export default {
  ...
  methods: {
    ...
    checkedList(row) {
      if (parseInt(row.invoice_status) == 1) {
        ...
        switch (parseInt(row.status)) {
          ...
          case 3: // 审核通过
            this.submitRent = true; // 提交
            this.auditRent = true; // 审核
            this.cancleRent = false; // 撤销审核
            this.createRent = false; // 账单
            this.cancellationRent = true; // 作废
            break;
        }
      }
    ...
    createCheck() { // 生成账单
      common.del('您将提交生成账单, 是否继续?', 'InvoiceDiscount/produceBill', { id: this.tableList.id },
        'InvoiceDiscount/getlist', this, 'rentList');
      this.cancel();
    },
    cancleList() { // 撤销审核
      common.del('您将提交撤销审核, 是否继续?', 'InvoiceDiscount/editStatus', { id: this.tableList.id, status: 1 },
        'InvoiceDiscount/getlist', this, 'rentList');
      this.cancel();
    },
    submitList() { // 提交审核
      common.del('您将提交审核, 是否继续?', 'InvoiceDiscount/editStatus', { id: this.tableList.id, status: 2 },
        'InvoiceDiscount/getlist', this, 'rentList');
      this.cancel();
    },
    auditList() { // 审核通过
      common.del('您将提交审核通过, 是否继续?', 'InvoiceDiscount/editStatus', { id: this.tableList.id, status: 3 },
        'InvoiceDiscount/getlist', this, 'rentList');
      this.cancel();
    },
    cancellation() { // 作废
      common.del('您将提交作废, 是否继续?', 'InvoiceDiscount/editStatus', { id: this.tableList.id, status: 0 },
        'InvoiceDiscount/getlist', this, 'rentList');
      this.cancel();
    },
    cancel() {
    ...
    isShow(data) { // 新增回调
      ...
      common.getPreData(list, 'InvoiceDiscount/getlist', this, 'rentList');
      common.set('rentList', { 'search': list });
      this.openRent = data;
    },
    pagination(val) { // 分页功能
      const data = this.formInline;
