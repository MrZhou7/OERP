...
<template>
  <el-main>
    ...
    <div class="btn_bottom">
      ...
      <el-button @click="cancle" :disabled="statusClick.cancle">取消收款</el-button>
      <el-button @click="confirmCancel" :disabled="statusClick.confirmCancel">确认取消收款</el-button>
    ...
    <!--账单收款-->
    <el-dialog title="收款" v-if="building" :fullscreen="true" :visible.sync="building" width="100%" height="60%" top="0%"
               :close-on-click-modal="false">
      <costRefund v-on:billAdminIndex="billAdminIndex" :cost="cost" :checkData="checkData"
                  :invoiceHeaderId="invoiceHeaderId" :costName="costName"></costRefund>
    </el-dialog>
    <!--查看-->
    <el-dialog title="查看" :visible.sync="costViewLists" width="80%" height="60%"
               :close-on-click-modal="false">
...
<script>
  ...
  export default {
    ...
    created() {
      this.getData()
      const searchHistory = common.get('receivablesListSearch')
      if (searchHistory != null) {
        this.formInline = searchHistory.search;
        this.pageSize = searchHistory.search.limit;
        common.getPreData(this.formInline, 'InvoicePayHeader/getList', this, 'receivablesListSearch')
      }
    },
    methods: {
      getData() {//预渲染参数
      ...
      onSecher() {
        ...
        common.getPreData(data, 'InvoicePayHeader/getList', this, 'receivablesListSearch')
        common.set('receivablesListSearch', { 'search': dataSearch })
      },
      clickRow(row) {//选择当前行
      ...
      //取消退款
      cancle() {
        this.open(this, 'Bill/cancelCollection', this.checkData.invoice_pay_id, 1, 'InvoicePayHeader/getList', 'receivablesListSearch')
      },
      //确认取消退款
      confirmCancel() {
        this.open(this, 'Bill/cancelCollection', this.checkData.invoice_pay_id, 2, 'InvoicePayHeader/getList', 'receivablesListSearch')
      },
      //撤销取消中
      rvokeCancel() {
        this.open(this, 'Bill/cancelCollection', this.checkData.invoice_pay_id, 0, 'InvoicePayHeader/getList', 'receivablesListSearch')
      },
      cancleAll() {
        this.statusClick.cancle = true //取消退款
        this.statusClick.confirmCancel = true//确认取消退款
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            that.http.post('InvoicePayHeader/editAccountDate', closeType).then(res => {
              ...
              common.getPreData(that.formInline, 'InvoicePayHeader/getList', that, 'receivablesListSearch')
            }).catch((err) => {
