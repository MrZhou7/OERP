...
export default {
  ...
  methods: {
    ...
    isPay() { // 付款确认
      ...
      that.http.post('order_pay/orderpay', { order_paying_id: JSON.stringify(that.order_paying_id), common: JSON.stringify(commonDteail), detail: JSON.stringify(detail) }).then(res => {
    ...
    getRadioTwo(row) { // 去除多选
      building.radioBtnTwo(row, this.$refs.tableSecond);
    },
