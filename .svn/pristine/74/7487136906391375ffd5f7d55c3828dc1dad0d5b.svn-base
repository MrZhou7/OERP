      ...
      <el-form-item label="品牌状态" prop="brand_status">
        <el-col :span="8">
          <el-select v-model="editTableData.brand_status">
            <el-option v-for="(item,index) in brandType" :key="index" :label="item.values_name" :value="item.values_code"/>
          </el-select>
        </el-col>
      </el-form-item>
...
export default {
  ...
  methods: {
    ...
    submitForm(editTableData) { // 保存
      this.$refs[editTableData].validate((valid) => {
        if (valid) {
          ...
          this.http.post('brand_info/editBrand', brandData).then((res) => {
    ...
    getEditBrand() { // 获取编辑品牌的信息
      ...
      this.http.post('brand_info/getBrandInfo', EditId).then((res) => {
