<template>
  <div id="businessWrap">
    ...
    <el-dialog
      ...
      width="90%">
      <EditBusiness :edit-data="editData" :edit_code="1" :check-bool="checkBool" @through="through"/>
    </el-dialog>
    <el-dialog
      v-if="dialogVisible"
      :close-on-click-modal="false"
...
export default {
  ...
  methods: {
    ...
    deleteRow(index, row) { // 作废
      common.del('此操作将作废该商户是否继续', 'customer/delCustomer', { customer_id: row[index].customer_id, enabled: 0 }, 'customer/getCustomerList', this, 'businessSearch');
    },
    startRow(index, row) { // 启用
      common.del('此操作将作废该商户是否继续', 'customer/delCustomer', { customer_id: row[index].customer_id, enabled: 1 }, 'customer/getCustomerList', this, 'businessSearch');
    },
