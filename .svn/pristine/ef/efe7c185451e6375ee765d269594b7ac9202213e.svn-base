  ...
  export default {
    ...
    methods: {
      onSecher() { // 查询
        ...
        common.getPreData(this.formInline, 'Repayment/getRepayMentList', this, 'refundsSlipManagement')
      ...
      // 去除多选
      getRadio(row) {
        common.radioBtn(row, this.$refs.table, this, 1);
      },
      checkedList(row) {
          ...
          if (parseInt(row.cancel_flag) == 0) {// 取消状态==未取消
            switch (parseInt(row.status)) {
              ...
              case 3:  //生效
                ...
                this.statusClick.rvokeCancel = false//执行
                this.statusClick.editTime = true//修改立账日期
                break
              default:
            }
      ...
      //取消执行
      cancle() {
        common.del('是否取消执行?', 'Repayment/cancelExecPepayBill', {
          id: this.checkData.id,
        }, 'Repayment/getRepayMentList', this, 'refundsSlipManagement')
      },
      //取消执行中
      confirmCancel() {
        common.del('是否确认取消执行?', 'Repayment/confirmCancelExecPepayBill', {
          id: this.checkData.id,
          user_id: this.checkData.id,
          mall_id: this.checkData.mall_id
        }, 'Repayment/getRepayMentList', this, 'refundsSlipManagement')
      },
      //执行
      rvokeCancel() {
        common.del('是否执行?', 'Repayment/execRepayBill', {
          id: this.checkData.id,
          user_id: this.checkData.id,
          mall_id: this.checkData.mall_id
        }, 'Repayment/getRepayMentList', this, 'refundsSlipManagement')
